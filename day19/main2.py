#!/usr/bin/env python3
# Not written by me - taken from the internet
# This is the one day I couldn't solve this year
...
def find_scanner(base, other):
    for scan in rotations(other):
...
def join(scans):
    ...
    remaining = deque(scans)
    scanners = [(0, 0, 0)]

    while remaining:
        scan = remaining.popleft()
        scanner, moved = find_scanner(joined, scan)
        if scanner:
            joined |= set(moved)
...
with open('./input') as f:
    joined, scanners = join(parse(f.readlines()))
print(len(joined))
